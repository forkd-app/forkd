...
WHERE
    CASE
        WHEN
            sqlc.narg('recipe_id')::uuid IS NOT NULL
            THEN sqlc.narg('recipe_id')::uuid = recipe_id
        ELSE TRUE
    END
    AND
    CASE
        WHEN
            sqlc.narg('parent_id')::uuid IS NOT NULL
            THEN sqlc.narg('parent_id')::uuid = parent_id
        ELSE TRUE
    END
    AND
    CASE
        WHEN
            ...
            THEN publish_date >= sqlc.narg('publish_start')::timestamp
        ELSE TRUE
    END
    AND
    CASE
        WHEN
            ...
            THEN publish_date <= sqlc.narg('publish_end')::timestamp
        ELSE TRUE
    END
    AND
    CASE
        WHEN
            ...
            THEN sqlc.narg('publish_cursor')::timestamp > publish_date
        ELSE TRUE
    END
    AND
    CASE
